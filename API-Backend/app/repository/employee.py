...
class EmployeeRepository:
    ...
    def insert(self,emp:Employee)->bool:
        try:
            self.sess.add(emp)
            self.sess.commit()
            current_app.logger.info('EmployeeRepository insert record')
            return True
        except Exception as e:
            current_app.logger.info(f'EmployeeRepository insert error: {e}')
        return False
    ...
    def delete(self, empid:str) -> bool:
        try:
           e=self.sess.query(Employee).filter(Employee.empid == empid).delete()
           self.sess.commit()
