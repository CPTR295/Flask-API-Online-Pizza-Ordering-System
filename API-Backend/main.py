...
class ImporovedJsonProvider(JSONProvider):
    def __init__(self, *args, **kwargs):
        self.options = kwargs
        super().__init__(*args, **kwargs)
    ...
    def load(self, s:str | bytes,**kwargs):
        s_dict:dict = json.load(s.decode('utf-8'))
...
class AppMiddleware:
    ...
    def __call__(self, environ , start_response):
        ...
        resp = werkzeug.wrappers.Response(start_response)
        print(f'Exiting URL endpoint: {api_path}')
        print(f'{resp.access_control_allow_origin}')
        return iterator 
    
app = create_app('../config_dev.toml')
app.json = ImporovedJsonProvider(app) 
...
def after_request_func(response:Response):
    ...
    app.logger.info(f'Exiting URL:{api_path} , function name:{api_endpoint_func}')
    return response
